...
func TestVerificationFollowsTheApplicantIntoTheGroup(t *testing.T) {
	...
	runFakeHandler(t, newAPITestBot(t, fb), v.OnMemberJoined, joinUpdate(gid, uid, ChatTypeSupergroup, nil))
...
func TestReleaseLeavesSomebodyElsesRestrictionAlone(t *testing.T) {
	...
	theirs := &fakeVerifyBot{member: &ChatMemberRestricted{Status: MemberStatusRestricted, IsMember: true, UntilDate: ours + 86400}}
	...
	v.pend[pkey{gid, uid}] = p
	mine := &fakeVerifyBot{member: &ChatMemberRestricted{Status: MemberStatusRestricted, IsMember: true, UntilDate: ours}}
...
func TestStaleAdminButtonDoesNotSettleTheNextVerification(t *testing.T) {
	...
	stale := Update{CallbackQuery: &CallbackQuery{
		ID:      "1",
		From:    User{ID: 42, LanguageCode: "en"},
		Message: &Message{MessageID: 1, Chat: Chat{ID: gid}},
