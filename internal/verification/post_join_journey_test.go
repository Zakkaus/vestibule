...
func memberDM(uid int64, text string) Update {
	return Update{Message: &Message{
		Chat: Chat{ID: uid, Type: ChatTypePrivate},
		From: &User{ID: uid, LanguageCode: "en"},
		Text: text,
	}}
}
...
func TestPostJoinJourneyPasses(t *testing.T) {
	...
	runFakeHandler(t, bot, v.OnMemberJoined, joinUpdate(gid, uid, ChatTypeSupergroup, nil))
	v.mu.Lock()
	...
	before := fb.sends
	runFakeHandler(t, bot, v.OnMemberJoined, joinUpdate(gid, uid, ChatTypeSupergroup, &ChatMemberRestricted{Status: MemberStatusRestricted, IsMember: true}))
...
func TestJoinRequestFallbackPassIsNotRepeatedAfterAdmission(t *testing.T) {
	...
	runFakeHandler(t, bot, v.OnJoinRequest, Update{ChatJoinRequest: &ChatJoinRequest{
		Chat: Chat{ID: gid, Type: ChatTypeSupergroup},
