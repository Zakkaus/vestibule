...
func newTestService(cfg *config.Config) *Service {
	...
	service := newService(settings, nil, &effective, &i18n.Messages)
	service.stateStore = testVerificationStore{}
	return service
}
...
func TestJoinResolvesApplicantAndGroupLanguagesSeparately(t *testing.T) {
	...
	update := Update{ChatJoinRequest: &ChatJoinRequest{
		...
		From: User{ID: userID, FirstName: "Applicant", LanguageCode: "zh-Hant"},
	}}

	runFakeHandler(t, newAPITestBot(t, bot), v.OnJoinRequest, update)
	wantGroup := v.messages.Verification.Group.Body.Render(
		i18n.LangZH,
		joinerLabel(userID, update.ChatJoinRequest.From.DisplayName(), v.NameSpoilerOn(groupID)),
		"",
		...
		t.Run(tt.code, func(t *testing.T) {
			for _, target := range []struct {
				...
				t.Run(target.name, func(t *testing.T) {
					...
					v.SendDMChallenge(context.Background(), uid, tt.code, target.groupID)
...
func TestJoinRequestDefaultPostsGroupBeforePrivateChallenge(t *testing.T) {
	...
	go func() {
		runFakeHandler(t, newAPITestBot(t, caller), service.OnJoinRequest, Update{ChatJoinRequest: &ChatJoinRequest{
			Chat: Chat{ID: groupID},
			From: User{ID: userID, FirstName: "Applicant", LanguageCode: "en"},
		}})
		close(done)
	}()
	defer func() {
...
func TestRequiredChannelPromptMessageIsRetainedForCleanup(t *testing.T) {
	...
	caller := newFakeVerifyBot()
	caller.member = &ChatMemberLeft{Status: MemberStatusLeft}
	runFakeHandler(t, newAPITestBot(t, caller), service.OnJoinRequest, Update{ChatJoinRequest: &ChatJoinRequest{
		Chat: Chat{ID: groupID},
		From: User{ID: userID, FirstName: "Applicant", LanguageCode: "en"},
	}})
...
func TestRequiredChannelDeepLinkPromptMessageIsRetainedForCleanup(t *testing.T) {
	...
	caller := newFakeVerifyBot()
	service.gateway = caller
	caller.member = &ChatMemberLeft{Status: MemberStatusLeft}
	bot := newAPITestBot(t, caller)
	runFakeHandler(t, bot, service.OnJoinRequest, Update{ChatJoinRequest: &ChatJoinRequest{
		Chat: Chat{ID: groupID},
		From: User{ID: userID, FirstName: "Applicant", LanguageCode: "en"},
	}})
	defer service.Shutdown()

	service.SendDMChallenge(context.Background(), userID, "en", groupID)
...
func TestJoinRequestBothCountsEitherConfirmedDelivery(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			errors:           map[int]error{2: apiError(403, "Forbidden: bot was blocked by the user")},
			wantDelivered:    true,
...
func TestJoinRequestDMModeDeliversPrivatelyOrFallsBackToScopedGroupLink(t *testing.T) {
	...
	t.Run("cannot initiate DM falls back with group-scoped link", func(t *testing.T) {
		service := newVerifier()
		caller := newFakeVerifyBot()
		caller.sendErr = apiError(403, "Forbidden: bot can't initiate conversation with a user")
		caller.sendFailN = 1
		runFakeHandler(t, newAPITestBot(t, caller), service.OnJoinRequest, update)
		defer service.Shutdown()
	...
	for _, rejection := range []struct {
		...
		{name: "blocked after an earlier conversation", err: apiError(403, "Forbidden: bot was blocked by the user")},
		{name: "rate limited without a retry delay", err: apiError(429, "Too Many Requests")},
	} {
...
// fakeVerifyBot is a Gateway stand-in so the approve / decline / ban handler branches can be
...
func (b *fakeVerifyBot) Probe(context.Context) error {
	b.getMeCalls++
	return b.getMeErr
}

func (b *fakeVerifyBot) recordSend(message OutgoingMessage) (int, error) {
	b.sends++
	sendN := b.sends
	b.lastSendChat = message.ChatID
	b.lastSendText = message.Text
	b.lastParseMode = ""
	if message.HTML {
		b.lastParseMode = "HTML"
	}
	b.sendChats = append(b.sendChats, message.ChatID)
	b.sendTexts = append(b.sendTexts, message.Text)
...
func (b *fakeVerifyBot) Send(_ context.Context, message OutgoingMessage) (int, error) {
	messageID, err := b.recordSend(message)
	return messageID, testGatewayError(err)
}

func (b *fakeVerifyBot) SendHTMLFallback(_ context.Context, chatID int64, rich, simpler string) (int, error) {
	send := func(text string, html bool) (int, error) {
		return b.recordSend(OutgoingMessage{ChatID: chatID, Text: text, HTML: html})
	}
	sent, err := send(rich, true)
	if err == nil {
		return sent, nil
	}
	if !markupRejected(err) {
		return 0, testGatewayError(err)
	}
	if simpler != "" && simpler != rich {
		sent, err = send(simpler, true)
		if err == nil {
			return sent, nil
		}
		if !markupRejected(err) {
	...
	sent, err = send(simpler, false)
	return sent, testGatewayError(err)
}
...
func (b *fakeVerifyBot) Notify(ctx context.Context, chatID int64, text string, _ int) {
	_, _ = b.Send(ctx, OutgoingMessage{ChatID: chatID, Text: text})
}

func (b *fakeVerifyBot) Alert(ctx context.Context, adminLogChatID int64, text string) {
	if adminLogChatID != 0 {
		_, _ = b.Send(ctx, OutgoingMessage{ChatID: adminLogChatID, Text: text})
	}
}
...
func (b *fakeVerifyBot) ApproveJoin(context.Context, int64, int64) error {
	b.approves++
	return testGatewayError(b.approveErr)
}

func (b *fakeVerifyBot) DeclineJoin(context.Context, int64, int64) error {
	b.declines++
	return testGatewayError(b.declineErr)
}

func (b *fakeVerifyBot) Ban(_ context.Context, _, _ int64, _ int, _ bool) error {
	b.bans++
...
func (b *fakeVerifyBot) AckFast(context.Context, string) error {
	b.answers++
	b.callbackAnswers = append(b.callbackAnswers, callbackAnswer{})
	return nil
}

func (b *fakeVerifyBot) AckResult(_ context.Context, _ string, result AckResult) error {
	b.answers++
	b.callbackAnswers = append(b.callbackAnswers, callbackAnswer{Text: result.Text, ShowAlert: result.Alert})
	return nil
}

func apiError(code int, description string) error {
	return fmt.Errorf("api: %d %q", code, description)
}

var testGatewayFailureMarkers = []struct {
	kind    FailureKind
	markers []string
}{
	{FailureJoinRequestGone, []string{"hide_requester_missing", "user_already_participant", "participant_id_invalid"}},
	{FailureApplicantGone, []string{"user is deactivated", "user_deactivated"}},
	{FailureCannotInitiateConversation, []string{"bot can't initiate conversation with a user"}},
	{FailureBlockedByUser, []string{"bot was blocked by the user"}},
	{FailureGroupUnreachable, []string{"bot is not a member of", "bot was kicked from", "chat not found", "the group chat was deleted"}},
	{FailureRateLimited, []string{"too many requests", "retry after"}},
}

var testGatewayErrorCodes = []struct {
	marker string
	code   int
}{
	{"429", 429},
...
func testGatewayError(err error) error {
	if err == nil {
		return nil
	}
	...
	message := strings.ToLower(err.Error())
	return &GatewayError{Cause: err, Kinds: testFailureKinds(message), Code: testFailureCode(message)}
}

func testFailureKinds(message string) FailureKind {
	var kinds FailureKind
	for _, failure := range testGatewayFailureMarkers {
...
func testFailureCode(message string) int {
	for _, candidate := range testGatewayErrorCodes {
...
func newAPITestBot(t *testing.T, gateway Gateway) Gateway {
	t.Helper()
	return gateway
}

func runFakeHandler(t *testing.T, gateway Gateway, handler Handler, update Update) {
	t.Helper()
	if err := handler(NewHandlerContext(context.Background(), gateway), update); err != nil {
...
func (b *blockingTerminalBot) ApproveJoin(context.Context, int64, int64) error {
	close(b.approveStarted)
	<-b.release
	return nil
}

func (b *blockingTerminalBot) DeclineJoin(context.Context, int64, int64) error {
...
func TestOnAnswer(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			bot := newFakeVerifyBot()
			if tt.required {
				bot.member = &ChatMemberLeft{Status: MemberStatusLeft}
			}
			update := Update{CallbackQuery: &CallbackQuery{
				ID: "answer", From: User{ID: tt.from}, Data: tt.data,
			}}
...
func TestOnAdminActionTelegramFailureAfterAcknowledgement(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			wantAlert := tt.wantAlert(v, errTelegram)
...
func TestTrustedBypass(t *testing.T) {
	...
	member.memberByID = map[int64]ChatMember{uid: &ChatMemberMember{}}
	...
	left.memberByID = map[int64]ChatMember{uid: &ChatMemberLeft{}}
	...
	fail.memberByID = map[int64]ChatMember{uid: &ChatMemberMember{}}
...
func TestJoinGate(t *testing.T) {
	...
	cooldown(v)
	bot := newFakeVerifyBot()
	bot.memberByID = map[int64]ChatMember{uid: &ChatMemberMember{}}
	...
	failBot.memberByID = map[int64]ChatMember{uid: &ChatMemberMember{}}
	...
	nonBot.memberByID = map[int64]ChatMember{uid: &ChatMemberLeft{}}
	...
	pBot.memberByID = map[int64]ChatMember{uid: &ChatMemberLeft{}}
...
func TestCooldownReapplicationDMsCurrentWait(t *testing.T) {
	...
	update := Update{ChatJoinRequest: &ChatJoinRequest{
		Chat: Chat{ID: gid},
		From: User{ID: uid, FirstName: "Applicant", LanguageCode: "zh-Hant"},
	}}
...
func TestDeliveredKernelPromptSurvivesRestart(t *testing.T) {
	...
	if !after.KernelAnswerDM(uid, answer.Message.Text, true) {
...
func TestLateBothDeliveryDeletesAbandonedMessagesWithoutPromptingReplacement(t *testing.T) {
	...
	update := Update{ChatJoinRequest: &ChatJoinRequest{
		Chat: Chat{ID: gid},
		From: User{ID: uid, FirstName: "Applicant", LanguageCode: "en"},
	}}
	release := make(chan struct{}, 1)
