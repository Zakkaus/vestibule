...
- **进程级凭据不进入每群设置。**`BOT_TOKEN`、 `VT_DATABASE_TYPE`、`VT_DATABASE_URI` 与 `TELEGRAM_API_URL` 在进程装配边界读取。未认领时， 认领口令只以哈希保存到 `STATE_DIRECTORY/claim.json`； 认领后 Bot token 以 `0600` 权限保存到同一文件，并删除该哈希。
