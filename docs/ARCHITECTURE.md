...
| GET /api/chats | 群与频道屏。返回该管理员可管理的群及可用群名（`{id, title?}`）。权限列表由 getChatMember 求交集。群名优先取运行时注册状态；缺失时首次通过 GetChat 查询，成功缓存一小时，失败（含查询超时）缓存 45 秒。每次查询上限三秒，同一群的并发查询合并；调用方取消只停止等待，不取消共享查询。缓存至多 4096 项，进程重启清空。配置群改名在缓存到期后的下一次查询反映；查询失败仍回退显示 ID。已注册群注销时删除群名，重新注册时刷新；注册期间改名不会主动刷新 |
