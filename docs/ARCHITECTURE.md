...
证书分三种情况，**每一种都要让人当场知道自己拿到的是哪一种**： 填了域名就自动申请；申请失败退回自签并说清浏览器会警告； 不填域名就只监听本机，并给出用端口转发打开控制台的那一条命令。 默认不把控制台暴露到公网。
...
| 无域名时签发短期 IP 证书并用 cron 续期 | **暂不取** | 它让没有域名的人也能拿到浏览器认的证书，确实解决了真问题。 但六天有效期加一条续期定时任务是一整条要长期维护的链， 这一轮先用只监听本机加端口转发，成本低且不会静默过期 |
