...
| Store | 8 | 四份 JSON 快照各一读一写。这一片不换介质、不动文件名与字段、 不调锁次序 —— 那些是阶段三的事，所以接口是现状的形状， 不是数据库的形状 |
...
- **进程级凭据不进入每群设置。**`BOT_TOKEN`、 `VT_DATABASE_TYPE`、`VT_DATABASE_URI` 与 `TELEGRAM_API_URL` 在进程装配边界读取。未认领时， `SETUP_TOKEN` 只以哈希保存到 `STATE_DIRECTORY/claim.json`； 认领后 Bot token 以 `0600` 权限保存到同一文件，并删除该哈希。
