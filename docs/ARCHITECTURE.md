...
**不把权限镜像进库。**每次敏感写入前用来访者的数字 ID 调用本地 Bot API 的 `getChatMember`。群设置、规则与撤销限制要求 `CanRestrictMembers`； 队列结算还要求 `CanInviteUsers` 与 `CanDeleteMessages`。 `creator` 具有该群全部权限。会话最长 8 小时，非写入路径每 60 秒复查管理员身份。 **被操作对象的管理员身份同样现查，不使用缓存。**
...
实例部署者是配置锁定并经一次性领取码认领的 `OwnerID`， 群拥有者则是 Telegram 的 `creator`，两者不互相授予权限。 部署者设置屏只允许当前非零 `OwnerID` 读取和调整群设置上限； 运维角色本身不构成授权，也不要求部署者先管理某个群。 群拥有者与管理员资料来自实时查询，不提供绑定或改绑。
