...
| GET /api/chats | 群与频道屏。返回该管理员可管理的群及可用群名（`{id, title?}`）。权限列表由 getChatMember 求交集；群名取运行时注册状态，不在请求时调用 Telegram。群注销时删除，重新注册时刷新。配置群与没有群名的旧记录回退显示 ID；注册期间在 Telegram 中改名不会主动刷新 |
