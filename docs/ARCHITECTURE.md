...
| 容器运行时 | 只在人选了容器部署、而机器上没有运行时的时候，才问要不要装。 **不问就装是越权** |
...
**容器里做不到同样的事，而假装做得到更糟。**容器内的进程要替换镜像， 必须能操作容器运行时，那等同于拿到宿主的最高权限。 为一个更新按钮交出这个权限不成比例。

| 部署方式 | 更新 |
|---|---|
| 原生服务 | 控制台点击即可，无需终端 |
| 容器 | 控制台显示有新版与变更说明，并给出要执行的那一条命令。 **按钮变成一条可复制的命令，不是灰掉的按钮** |

界面上写清为什么这里不同， 而不是让人以为功能坏了。
