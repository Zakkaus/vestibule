...
**验收**：`scripts/lint.sh` 可执行；故意把一个文件写到 601 行时它变红，
且红在行数检查那一条上；基线清单里的每一项**在冻结那一刻**都能追到具体文件与行数。
...
原先这一条写的是 `make lint`。**仓库里没有 Makefile，从写下那天起就没有过** ——
这条验收因此从来无法执行，而阶段零标着完成。入口是 `scripts/lint.sh`，
贡献指南的门禁清单里一直是它。
...
**验收**：Mini App 的 `initData` 换得到会话，过期与重放各拒一次；
写入端点缺 `X-CSRF-Token` 返回 `csrf_invalid`；
不在授权群列表里的群号返回 `chat_not_found` 而不是空集合；
接口层不出现判定逻辑 —— 结算走 `verification.Service`，不直接写 SQL。

**这一条是补写的。** 发现它的那一轮，全部阶段里只有这一节没有验收条款，
`grep -n '^\*\*验收' docs/PLAN-v5.md` 数出来是十条不是十一条，
而阶段五早已标为完成。上面四句取自本节「必须保住的行为」与
「接口层不做判定」，都是这一阶段已经交付并有测试覆盖的性质，
不是新要求。
...
**验收**：架构文档第 11 节的路由表与实现一致（**这一条原先写的是「契约生成物与手写代码无差异」，
而仓库里没有 OpenAPI 规格、没有生成器、没有生成物目录，也没有比对命令 ——
那条验收从写下那天起就无从执行**。架构文档第 302 行自己写着这份契约是从代码反推的，
所以「契约」在这个仓库里就是那张路由表）；前端在没有后端时能对着假数据启动；
这条通路有一个跑在真浏览器里的端到端用例，从读取已有会话或 Mini App 换取会话走到放行成功。
...
|---|---|
| 最小通路走得通 | 已达成 |
| 端到端用例在真浏览器里通过 | 已达成。浏览器先读取已有会话；只有 Mini App 有 `initData` 且没有会话时才换取会话 |
| 每条路由 × 两宽度 × 三主题 × 最宽语言，加键盘走一遍 | 已达成，且已机械化 —— `web/e2e/render-gate.spec.ts`，CI 在执行 |
| 一条命令部署后健康检查通过 | **等阶段九**。HTTP 服务已有 `/readyz`；容器化部署与安装后的健康检查归阶段九 |
| **实机**：控制台连测试环境的库，放行测试群里一个真实申请 | **等阶段九**。四屏都已连接口，放行会真的发出结算；缺的只是一套部署好的测试环境 |

前三条由本阶段自己负责，都已做到。后两条写在这里是为了记住它们属于这条通路；
它们剩下的前提只有阶段九的部署，本阶段造不出来。
...
**端点的形状不在这里写。** 上一轮已经吃过隔着代码写签名的亏：
形状由调用点决定，派工时只给不变量（按群授权、写前现查管理员、
revision 不匹配要能被前端区分成「别人改过了」而不是「保存失败」），
让实现的人从 `Store` 的调用面推出来再写回文档。
...
**这不是端点漏了字段，是这两屏管的设置本来就不按群分。****

所以它们要的是另一个口：进程层设置的读写，或者只读暴露。
形状同样不在这里写 —— 先做那五个能做的屏，做的过程里会看清进程层该以什么形式出现；
现在定它，就是又一次隔着代码写签名。

**消息与文案屏也过不去，缺的是另一个口。**** 它管五项，
只有显示名剧透、自动撤回（含延时）、富文本在设置里；
**自动回复规则根本不在设置层，在 `rule` 表**
（`migrations/00-latest.sql:58-65`：`id`、`chat_id`、`collection`、`ordinal`、
`enabled`、`definition`），进出提示与验证全流程文案也没有字段。
所以它要的是一个规则表的读写口，不是设置端点上再加字段。

**那一片比设置端点那片重**：`rule` 表由阶段三的迁移建了出来，
而到今天**没有任何代码读写它** —— `internal/database/` 下一处 `FROM rule` 都没有。
设置端点那片能小，是因为 `Store.Settings` 与 `Store.Update` 早就在了，
只差把它们接出来；规则那片要连存储层一起做。
派它的时候按这个规模给时间，别按前几个屏的规模估。
...
**实机（未做，要维护者点头）**：测试群与另一个临时群同时挂在同一个进程上，
两边配置互不影响。上面那三条已由测试证明，这一条要真的挂两个群到一个进程上，
碰的是生产的机器人账号，不在无人值守的授权范围内。
...
| 进行中的待验证记录（`pending.json`） | `cmd/import-state` —— **但本阶段不该迁**，见下 | 切换窗口内它们都会超时，迁过去反而会误判 |

**这里有一处实现与本节矛盾，切换之前必须解决。** 上面最后一行写着「不迁」，
而 `cmd/import-state` 无条件迁 `pending.json`（`internal/database/import.go:194`、
`:230` 的 `replacePending`），**没有跳过它的开关**。两种解法各有代价：
给命令加一个跳过开关，代价是多一个只在切换那天用一次的旗标；
或者承认迁过去无害（新实例接手后它们同样会超时），代价是与本节写死的理由相反。
**这一条要维护者点头**，不由实施方自行决定。

原先这张表里还有一行「已通过验证的名单」，写着丢了会让老成员重新做题。
**上一代不存这样一份名单** —— 七份文件里没有它，成员身份在 Telegram 那边，
而验证只对入群申请触发，已在群里的人不会被要求做题。那一行已删除。
...
2026-09-02 维护者一次性答完了积压的待决，答案落在这里。这些**都不是重写的一部分** ——
重写做完就是上一代的能力加控制台；下面是他另外要的。放在切换之后，
因为阶段十的判据是「与上一代等价」，而这些是加法；哪一条要提前，他说一声即可。

**主题。**默认黑白（Graphite），另外支持 Catppuccin Frappé、Catppuccin Macchiato、
Catppuccin Mocha、Tokyo Night Storm、Tokyo Night。偏好屏原先欠着的「强调色」由此取代 ——
不做一个取色器，做一组配色。六套都要过现有的对比度与主题泄漏检查。

**再加两种语言：日语、俄语。**目录从三份变五份。
注意俄语的复数类别不是 `one`/`other` 两种，现有的复数键要按语言补齐类别。

**渲染门禁跑满所有语言，但不必每次。**维护者原话是「可以都试试看但是不用太频繁」。
所以按次数分两档：每个 PR 跑实测最宽的那一种，定时跑全部语言 × 全部屏。
这一条同时了结「三种语言各自跑一遍还是只跑最宽」那条待决。

**拥有者绑定是唯一的。**一个群一个拥有者，不是一组。这一半已经成立：
`ClaimOwner`（`internal/settings/store_registration.go:96`）只在还没有拥有者时接受认领，
第二个人来认领同一份领取码会被拒，测试护着这两条。
**本阶段欠的是控制台那一侧** —— 接口不返回拥有者，群与频道屏也就无从显示或改绑。

**控制群要做。**管理与处罚屏欠的那条。

**试答要做。**题库屏欠的那条，对应架构文档路由表里的 `POST /api/chats/{id}/rules/test`。
它调用线上同一份判定代码 —— 这一点原先被误写进阶段三的「必须保住的行为」，
而上一代没有 HTTP 服务，它是新功能。

**设备状况可以每日推送，且可关闭。**诊断屏那边的事实已经有了，缺的是投递与开关。

**结构信号要补。**架构文档里那套按消息实体计分的反垃圾，两代代码里都不存在，
原先记着「没有任何阶段排它」，现在归本阶段。

**内置模板**（题库屏欠的另一条）与**标题图标**（偏好屏欠的另一条）**不在本阶段**：
维护者没有点名它们。要么之后单独提，要么从设计文档里去掉，仍是待决。

**验收**：六套配色各自在两个宽度下渲染无溢出且对比度达标；五种语言的目录键集与占位符一致，
俄语复数类别齐全；定时任务跑满语言 × 屏并留下记录；一个群只能绑定一个拥有者，
绑第二个被拒绝；控制群、试答、日报开关各有一条端到端用例；
结构信号有样本与断言，且关掉它时不影响其余反垃圾。
...
| 问题 | 哪个阶段逼着定 | 影响 |
|---|---|---|
| 控制台域名 | 见下面「一条没有排期的」 | Mini App 的配置与证书都要它。**原先写的是阶段五，那一阶段已经完成而这个问题没被问出来** ——因为实现把它做成了配置（`CONSOLE_URL`，留空即不投递链接，Mini App 登录接口照常），所以代码不需要答案。真正需要它的是部署：证书要签给某个域名，Mini App 也要填一个。
安装脚本把域名当作它询问的四件事之一并带默认值，所以脚本写得出来；
签不出证书的是**在一台真机上走一遍**那一条 |
| 把一个群从配置里去掉，它进行中的验证要不要保住 | 阶段十 | 实测：去掉配置再重启，那个群的 `pending` challenge 会被恢复流程改成 `superseded`。**数据库的行都还在**，`challenge`、`pending_action`、`rule`、`verification_failure`、`warning_counter` 一行不少，别的群也无法读取它们；但把这个群加回来之后队列是空的，进行中的那几个人不会自动接上。这在阶段十逼着定：那一阶段要「随时可退回」，而退回一次就是把群从这个实例的配置里去掉一次 |
| ~~申请人的应答要不要提前到动作之前~~ **已定 2026-09-02：提前。** | —— | 实测：管理员点按钮约 1.0 秒看到确认，申请人答对约 1.5–2.0 秒。差别在于申请人那条路上，`ApproveJoin`、`Delete` 串在应答之前。架构第 7 节定的是「先判定，再应答」——判定是本地的纯函数，`ApproveJoin` 是动作不是判定，所以应答本可以排在判定之后、动作之前，约 0.5 秒。**这一条在阶段三之前不安全**：应答说「通过了」而随后动作失败，就是撒谎。阶段三第三片之后动作与状态转换同事务落库、由执行器重试，这个前提变了。代价是应答说的是「已判定通过」而不是「已经进群了」，文案要跟着改 |
| 迁移要不要带上进行中的待验证记录 | 阶段十 | **开关已经做出来，选择仍待定。** `cmd/import-state -pending carry\|drop` 是必填的，不给就拒绝执行，而且在备份与写入之前就拒绝。两种代价：`carry` 把旧机器人**仍在结算**的那批挑战写进新库（本阶段写明旧机器人整个观察期不停机），同一个申请人可能被两个机器人各处理一次；`drop` 把那批申请人留给仍在应答他们的旧机器人，备份里一条不少。证据偏向 `drop`，但这是你的决定，命令不替你选 |
