...
本阶段又做完了偏好屏、操作记录屏、管理与处罚屏、验证方式屏、免验证来源屏、题库屏、订阅推送屏、统计屏、消息与文案屏和功能屏，还剩 3 个。阶段六还建了进入屏，但它不在那 15 个里：
...
| 诊断 | `/livez` `/readyz` 已有；权限预检、Bot API 延迟、心跳还没有对应端点 |
| 版本 | **等阶段九**。当前版本、可回退与升级都由那一阶段的部署机制提供 |
...
| 申请人的应答要不要提前到动作之前 | 阶段七（进行中，不阻塞） | 实测：管理员点按钮约 1.0 秒看到确认，申请人答对约 1.5–2.0 秒。差别在于申请人那条路上，`ApproveJoin`、`Delete` 串在应答之前。架构第 7 节定的是「先判定，再应答」——判定是本地的纯函数，`ApproveJoin` 是动作不是判定，所以应答本可以排在判定之后、动作之前，约 0.5 秒。**这一条在阶段三之前不安全**：应答说「通过了」而随后动作失败，就是撒谎。阶段三第三片之后动作与状态转换同事务落库、由执行器重试，这个前提变了。代价是应答说的是「已判定通过」而不是「已经进群了」，文案要跟着改 |
