...
编译通过和测试全绿证明不了机器人在群里的行为。**从有可跑的东西开始，
...
**两项前置还没做，做完之前验证一次都跑不起来**，因为验证只在加群申请这条路上触发：

| 要做什么 | 现状 | 不做会怎样 |
|---|---|---|
| 把机器人提为测试群管理员，给「邀请用户」「封禁用户」「删除消息」 | 现在是普通成员 | 收不到加群申请，也没法通过、拒绝、禁言、删挑战消息 |
| 打开群的「需要管理员批准」 | 未开 | 根本不会产生加群申请事件，机器人等不到任何东西 |

这两项只能由群主在 Telegram 里点，接口做不到。**提为管理员之后隐私模式那一条也一并解决**：
管理员身份的机器人本来就收得到群里全部消息，反垃圾才有得测。
...
一条命令部署后健康检查通过。
**实机**：控制台连的是测试环境的库，放行的是测试群里一个真实申请。
...
这一阶段同时把 Gentoo 与 Linux 相关的查询和订阅拆成可关闭的模块：
...
迁完先核对：两边各自数一遍上面每一类的条数，对不上就停下来查，不靠感觉。
...
- 十一个阶段全部合入 `next` 并稳定运行后，一次发布 v5.0.0。
...
| 问题 | 影响 |
|---|---|
| 控制台域名 | 影响 Mini App 配置与证书 |
| 隐私说明的内容与位置 | 公开实例保存他人群组数据，须在明处说明 |
| 公开实例用哪个机器人账号 | `@GentooZhVerifyBot` 带社区名，给所有人用不合适；换号涉及老群迁移 |
| 分支合入路径：建 `next` 还是直接合 `main` | 计划书各阶段都写着合入 `next`，而这个分支不存在。不定就会一直堆分支 |
| 控制台的访问入口 | 设计书要求只在 Telegram 内打开；架构书要求普通浏览器走 Telegram Login OIDC。两条不能同时成立 |
| 受众模型 | 设计书把使用者收敛为群管理员；架构书把运维与群管理员设为两类主体。决定界面按能力过滤还是按角色分屏 |

### 设计书与架构书对不上，我来定

一次一致性检查列出十二条，其中令牌录入位置与容器更新按钮已在本轮定死，
配置包命令那条是架构书自相矛盾、已修。以下九条还开着：

| 冲突 | 两边各说了什么 |
|---|---|
| 首次认领路由 | 设计书用一次性认领链接加私聊口令；架构书只定义了 Mini App、OIDC 与一个公开验证面 |
| 设置值的来源 | 设计书展示默认、群值、待提交三种；架构书把待提交与来源分开，来源另有 `provisioning` 与 `locked` |
| 群模式的展示 | 架构书要求群列表显示当前 Telegram 模式；设计书没有对应字段与实例 |
| `membership` 的归属 | 设计书说外部成员资格不是挑战；架构书把它列进 `challenge.kind` |
| 挑战发送位置 | 设计书允许群内、私聊、网页三处；架构书主流程固定私聊，数据模型也没有这个字段 |
| 反垃圾算不算规则集合 | 设计书纳入统一规则体系；架构书的 `rule.collection` 只有三种，配置包表却允许 `antispam` |
| 屏与接口的覆盖 | 设计书有统计、诊断、配置包、偏好、版本等屏；架构书路由表未覆盖其中多项 |

每条都要落到一份文档上，**不是两边各改一半**。改完两份都要重新过一致性检查。
