...
**升级顺序里最要紧的一步在下载之前：这一片已落实。**`migrations.AssessRollback` 从嵌入迁移表的兼容下限计算目标结构与回退结构的关系；`FetchAfterRollbackCheck` 在不可回退时返回带版本下限的原因，绝不调用取回函数。
实际下载和替换仍归后续片完成；装完才发现回不去等于没有回退。

**但那两个函数装不进安装脚本，中间还缺一步。** 迁移表由 `//go:embed *.sql`
在编译期嵌入（`migrations/migrations.go:10`），所以**一个二进制只知道它自己那一份**：
已装的旧版判断不了尚未下载的新版要求什么，而新版还没下载。
发布流程目前只发二进制与 `SHA256SUMS`（`.github/workflows/release.yml:53`、`:59`），
