...
**2 个来源文件。** 原清单误将自称模型计数列为结构信号；
当前对应实现位于 `internal/verification/state.go:19`：
...
**试答。** 题库屏已提供 `POST /api/chats/{id}/rules/test` 对应的试答入口，
只读取已保存设置中的有效 `Questions` 或 `FallbackQuestions`，并调用生产路径的同一份答案判定代码；
试答不读取编辑草稿，也不写入验证、审计或统计状态。该项曾误列为阶段三必须保留的行为；
