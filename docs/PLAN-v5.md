...
| 九 | `v5/deploy` | 一次部署、健康检查、失败自动回退 | 进行中 |
...
而 `cmd/import-state` 无条件迁 `pending.json`（`internal/database/import.go:152`、
...
| 问题 | 哪个阶段逼着定 | 影响 |
|---|---|---|
| 控制台域名 | 阶段九（进行中，**不阻塞写代码，阻塞实机**）| Mini App 的配置与证书都要它。**原先写的是阶段五，那一阶段已经完成而这个问题没被问出来** ——因为实现把它做成了配置（`CONSOLE_URL`，留空即不投递链接，Mini App 登录接口照常），所以代码不需要答案。真正需要它的是部署：证书要签给某个域名，Mini App 也要填一个。
...
| 迁移要不要带上进行中的待验证记录 | 阶段十 | 本节写着不迁，`cmd/import-state` 无条件迁（`internal/database/import.go:188`），没有跳过开关。加开关还是改理由，两种代价都写在阶段十那一节 |
