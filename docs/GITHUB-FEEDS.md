...
提交继续使用原有基线和游标规则。issue 与 pull request 各有独立的首次启用标记和编号游标；某类首次启用时以当前页内该类的最高编号建立基线，不补发历史。后续条目按编号升序发送。成功发送的条目会记录原消息 ID 与当前状态；首次发现已合并或已关闭的条目会直接按该状态发送。关闭、合并和重开只就地编辑原消息，不发送新的状态消息；重开持久化为 `open`，只渲染一次「已重开」。
...
评论与 review 事件仍不发布。每个仓库最多跟踪 100 条成功发送的 issue 或 pull request，满额时先淘汰终态条目；只更新仍在当前 30 条 REST 页面内的跟踪条目，页面外条目会保留但不更新。GitHub 与 Bugzilla 每目的地每轮共用 20 条编辑预算。`message is not modified` 会同步状态，`message to edit not found` 或 `message can't be edited` 会立即停止跟踪；其他可计数的 `400` 连续 10 次后停止跟踪，`429` 会停止该目的地本轮编辑并保留未更新状态。GitHub 源仍受 `gentoo` 模块门控。GitHub Enterprise 的版本、匿名权限、路由、重定向、Atom 标识及 REST 路径尚未端到端核实。
